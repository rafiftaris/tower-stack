...
import {BUTTON_TYPE, DIRECTION} from "../Enum/enum";

import AlignTool from "../Util/AlignTool";
import { 
    TextPopUp, 
    ANIMATION_TYPE as TEXT_ANIM_TYPE 
} from "../Util/TextPopUp";
import {
    ImagePopUp,
    ANIMATION_TYPE
} from "../Util/ImagePopUp";
...
export default class HowToScene extends Phaser.Scene {
    ...
    private bird: Bird;
    private backToTitleButton: PlayButton;
    ...
    create(): void {
        ...
        this.time.addEvent({
            ...
            callback: () => {
                this.goalDescription = TextPopUp.showText({
                    ...
                    text: "Stack the box as high as you can.\n\nThe higher the box,\nthe greater the score you get!",
        ...
        this.time.addEvent({
            delay: (this.titleTransTime + this.goalTransTime + this.transDelay*2) * 1000,
            callback: () => {
        ...
        this.time.addEvent({
            delay: (this.titleTransTime + this.goalTransTime + 
                this.itemDescTransTime + this.transDelay*3) * 1000,
            callback: () => {
                this.tweens.add({ 
        ...
        this.time.addEvent({
            delay: (this.titleTransTime + this.goalTransTime + 
                this.itemDescTransTime*2 + this.transDelay*4) * 1000,
            callback: () => {
        ...
        this.time.addEvent({
            delay: (this.titleTransTime + this.goalTransTime + 
                this.itemDescTransTime*3 + this.transDelay*5) * 1000,
            callback: () => {
                this.tweens.add({  
                    targets: this.bird,
                    x: AlignTool.getXfromScreenWidth(this,0.85),
        ...
        // Button
        this.backToTitleButton = new PlayButton(
            this,
